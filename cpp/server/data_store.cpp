...
mini2::ServiceRecord DataStore::toProto(size_t i) const {
    ...
    sr.set_closed_date(closed_date_[i]);
    sr.set_due_date(due_date_[i]);
    sr.set_resolution_updated_date(res_updated_date_[i]);
    sr.set_incident_zip(incident_zip_[i]);
    sr.set_bbl(bbl_[i]);
    sr.set_community_board(community_board_[i]);
    sr.set_council_district(council_district_[i]);
    sr.set_police_precinct(police_precinct_[i]);
    sr.set_x_coord(x_coord_[i]);
    sr.set_y_coord(y_coord_[i]);
    sr.set_latitude(latlon_[i].lat);
    sr.set_longitude(latlon_[i].lon);
    auto sv = [](StringRef r, const StringPool& p) { return std::string(p.get(r)); };
    ...
    sr.set_status(std::string(reg_status_.decode(status_code_[i])));
    sr.set_open_data_channel(std::string(reg_channel_.decode(channel_code_[i])));
    sr.set_borough_name(std::string(reg_borough_.decode(borough_code_[i])));
    sr.set_park_borough(std::string(reg_park_borough_.decode(park_borough_code_[i])));
    sr.set_additional_details(sv(additional_details_[i], *main_pool_));
    sr.set_incident_address(sv(incident_address_[i], *main_pool_));
    sr.set_street_name(sv(street_name_[i], *main_pool_));
    sr.set_cross_street_1(sv(cross_street_1_[i], *main_pool_));
    sr.set_cross_street_2(sv(cross_street_2_[i], *main_pool_));
    sr.set_intersect_street_1(sv(intersect_street_1_[i], *main_pool_));
    sr.set_intersect_street_2(sv(intersect_street_2_[i], *main_pool_));
    sr.set_city(sv(city_[i], *main_pool_));
    sr.set_landmark(sv(landmark_[i], *main_pool_));
    sr.set_resolution_desc(sv(resolution_desc_[i], *res_pool_));
    sr.set_park_facility(sv(park_facility_[i], *main_pool_));
    sr.set_vehicle_type(sv(vehicle_type_[i], *main_pool_));
    sr.set_taxi_company_borough(sv(taxi_company_borough_[i], *main_pool_));
    sr.set_taxi_pickup(sv(taxi_pickup_[i], *main_pool_));
    sr.set_bridge_hwy_name(sv(bridge_hwy_name_[i], *main_pool_));
    sr.set_bridge_hwy_dir(sv(bridge_hwy_dir_[i], *main_pool_));
    sr.set_road_ramp(sv(road_ramp_[i], *main_pool_));
    sr.set_bridge_hwy_seg(sv(bridge_hwy_seg_[i], *main_pool_));
    sr.set_location(sv(location_[i], *main_pool_));
    return sr;
}
